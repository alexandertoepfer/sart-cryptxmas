...
unsigned char a[] = {
    ...
    0x66,0x67,0x68,0x69,0x6a,0x6b,0x6c,0x6d,0x6e,0x6f,0x70,0x71,0x72,0x73,0x74,0x75, // E
...
unsigned char s[] = {
    ...
    0x7a,0x29,0x2b,0x3d,0x55,0x21,0x73,0x34,0x3c,0x67,0x72,0x64,0x62,0x3a,0x68,0x00, // K
...
int main() {
    ...
    // In and out for the hash function -> {0,1,...,17,18='\0'}.
    unsigned char in[19], out[19];
    string outputString = "2V]0>V.IyE?cHFv3|?";

    // f(in) -> out.
    string inputString = "thisIsATest12345!?"; // j
    if(inputString.size() > 18) throw new runtime_error("Length > 18");
    strcpy((char*)in, inputString.c_str());
    in[18] = out[18] = '\0';

    unsigned char* r = f(in);
    for(int i = 0; i < Q; i++)
        out[i] = r[i]; // e

    // Example #1
    cout << "f(in=\"" << inputString << "\") -> out=\"" << toString(out) << '"' << endl; // Q
    cout << "f(in=\"XXXXXXXXXXXXXXXXXX\") -> out=\"" << outputString << '"' << endl << endl;

    cout << "Input found?" << endl << boolalpha << (toString(out) == outputString);
    // True => You found the reverse of the hash!
    // Use the text as password for the zip file!
    return 0;
}
